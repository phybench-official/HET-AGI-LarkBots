...
def with_tools_func_factory(
    ...
    async def workflow_func(
        ...
        raw_response = await get_answer_async(
            prompt = problem_text,
            model = lark_bot._config["with_tools"][model]["model"],
            ...
            temperature = lark_bot._config["with_tools"][model]["temperature"],
            timeout = lark_bot._config["with_tools"][model]["timeout"],
            trial_num = lark_bot._config["with_tools"][model]["trial_num"],
            trial_interval = lark_bot._config["with_tools"][model]["trial_interval"],
            tools = [
                mathematica_tool(
                    timeout = 60,
                    verbose = True,
            ...
            tool_use_trial_num = lark_bot._config["with_tools"][model]["tool_use_trial_num"],
        )
